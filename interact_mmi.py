...
def set_interact_args():
    ...
    parser.add_argument('--mmi_model_path', default='data/chitchat_v1/best_acc_model.cpt', type=str, required=False,
                        help='互信息mmi_model路径')
...
def main():
    ...
    tokenizer = BertTokenizer.from_pretrained('data/pretrained_model/gpt2-chinese')
    # 对话model
    dialogue_model = GPT2LMHeadModel.from_pretrained('data/pretrained_model/gpt2-chinese')
    dialogue_model.to(device)
    dialogue_model.eval()
    # 互信息mmi model
    mmi_model = GPT2LMHeadModel.from_pretrained('data/pretrained_model/gpt2-chinese')
    mmi_model.load_state_dict(torch.load('data/chitchat_v1/best_acc_model.cpt', map_location=torch.device('cuda')))
    mmi_model.to(device)
    mmi_model.eval()
